...
import { badgeUrl, skillEarned, norm, GAME_CATALOG, GEAR_BADGES, SKILL_CATALOG, NEWEST_BATCH, isNewSkill, skillProblem, PROBLEM_BADGES } from '../src/catalog.js'
...
// Badge bermasalah (challenge lab error/deprecated) tetap di katalog — poinnya sah kalau selesai —
// jadi id-nya harus menunjuk entri yang benar-benar ada, bukan typo yang diam-diam tidak menandai
// apa pun.
test('badge bermasalah: semua id ada di SKILL_CATALOG dan punya alasan', () => {
  const ids = new Set(SKILL_CATALOG.map((s) => s.id))
  for (const [id, { why }] of Object.entries(PROBLEM_BADGES)) {
    assert.ok(ids.has(Number(id)), `badge bermasalah ${id} tidak ada di SKILL_CATALOG`)
    assert.ok(why && why.length > 10, `alasan badge ${id} kosong/kepotongan`)
  }
})

test('skillProblem: badge normal null, badge bermasalah punya alasan', () => {
  assert.equal(skillProblem(728), null)                      // Pub/Sub, normal
  assert.equal(skillProblem(640).length > 0, true)           // Cloud Architecture, bermasalah
})
