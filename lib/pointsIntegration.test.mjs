...
import { computePoints, FACIL_END } from './parseProfile.js'
import { scoreProfile, MS, BONUS_TASK, DEADLINE } from './points.js'
...
// Penjaga kelas bug: saat DEADLINE diperpanjang 14 -> 29 Sep 2026, FACIL_END tertinggal di 14 Sep
// sehingga game yang di-earn 15-29 Sep tidak mengisi milestone. Peserta melihat jumlah game di
// kartu (13) berbeda dari bar milestone-nya (11) dan progresnya seolah mandek.
test('akhir window milestone ikut DEADLINE, bukan mentok di tanggal tutup pendaftaran lama', () => {
  assert.equal(FACIL_END.getTime(), DEADLINE.getTime(), 'FACIL_END wajib sama dengan DEADLINE')

  const setelahPerpanjangan = new Date('2026-09-20T10:00:00+07:00') // antara 14 Sep dan 29 Sep
  const p = computePoints([badge(GAME_BIASA, setelahPerpanjangan)])
  assert.equal(p.facilGames, 1, 'game 15-29 Sep wajib mengisi milestone')
})
