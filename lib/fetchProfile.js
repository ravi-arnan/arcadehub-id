...
export async function fetchAndScore(profileUrl) {
  ...
  // 0 badge = profil privat / kosong / markup berubah, jangan diam-diam simpan skor 0.
  if (badges.length === 0) throw new Error('Tidak ada badge terbaca. Pastikan profil di-set PUBLIC dan link benar.')
  const p = computePoints(badges)
  const sc = scoreProfile(p.seasonGames, p.seasonSkills, p.facilGames, p.facilSkills)
  return {
    name,
