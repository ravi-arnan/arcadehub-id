...
const A_GAME = 'Pitch Perfect'                                       // Arcade Special Sep 2026
...
// Dua judul ini benar-benar ada di profil peserta (dipanen dari 79 profil publik, Sep 2026) dan
// keduanya LOLOS dari deteksi sebelum diperbaiki: tidak memuat kata "Arcade", dan "Cloud Canvas"
// tidak punya entri di PAST_GAMES. Akibatnya badge game kehitung 0 poin dan jumlah game peserta
// lebih kecil dari yang terlihat di profilnya.
test('classifyBadge: game seri "Work Meets Play" tanpa kata "Arcade" tetap terbaca game', () => {
  const c = classifyBadge('Work Meets Play: Cloud Canvas')
  assert.equal(c.kind, 'game')
  assert.equal(c.counts, true)
})

test('classifyBadge: "Work Life Refresh" (Januari) terbaca game walau tanpa awalan Arcade', () => {
  const c = classifyBadge('Work Life Refresh')
  assert.equal(c.kind, 'game')
  assert.equal(c.counts, true)
})

// Game spesial Juni 2026. Profil peserta menulis "Logic Log", sedangkan arsip resmi Juni cuma
// menulis label generik, jadi judul ini tidak pernah masuk PAST_GAMES. Dibandingkan dengan
// ArcadeCalc (levelBadge, 1 poin) di 20 profil: ini satu-satunya game yang belum kita hitung.
test('classifyBadge: "Logic Log" (Juni) terbaca game', () => {
  const c = classifyBadge('Logic Log')
  assert.equal(c.kind, 'game')
  assert.equal(c.counts, true)
})

// DIKONFIRMASI Google Skills Support lewat arcade-facilitator@google.com (20 Sep 2026): tiga
// course "Google Cloud Database Migrations" bukan Skill Badge, peserta dapat Course Completion
// Badge. Sempat dihitung 0,5 poin selama beberapa jam, lalu dicabut. Tes ini menjaga supaya
// tidak ada yang menambahkannya lagi ke SKILL_CATALOG hanya karena ArcadeCalc menghitungnya.
test('"Google Cloud Database Migrations" BUKAN badge keahlian (course completion badge)', () => {
  const titles = [
    'Google Cloud Database Migrations: Strategy',
    'Google Cloud Database Migrations: Homogeneous',
    'Google Cloud Database Migrations: Heterogeneous',
  ]
  for (const t of titles) {
    const c = classifyBadge(t)
    assert.equal(c.kind, 'unknown', t)
    assert.equal(c.counts, false, t)
  }
})
