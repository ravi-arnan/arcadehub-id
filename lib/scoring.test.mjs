...
// PROVISIONAL, 20 Sep 2026. Ketiganya dihitung badge keahlian atas keputusan Ravi sambil menunggu
// jawaban resmi Google (kartu skills.google bilang "SKILL BADGE", tapi data katalognya
// `credentialType: null`). Hapus tes ini bersama ketiga entri di SKILL_CATALOG kalau jawabannya
// "bukan badge keahlian".
test('provisional: tiga credential "Google Cloud Database Migrations" dihitung badge keahlian', () => {
  const titles = [
  ...
  for (const t of titles) {
    ...
    assert.equal(c.kind, 'skill', t)
    assert.equal(c.counts, true, t)
  }
})
